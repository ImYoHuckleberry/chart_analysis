        ...
        # --- MODIFY REGION ---
            ...
            if region_list:
                ...
                with sidebar_cols[0]:
                    ...
                    if st.button("Use Start Date", key="btn_edit_start_idx"):
                        idx_match = df[df['time'].str[:10] == str(start_date)].index
                        if not idx_match.empty:
                            st.session_state[edit_start_idx_key] = idx_match[0]
                        st.rerun()
                with sidebar_cols[1]:
                    end_idx = st.number_input("End Index", min_value=start_idx + 1, max_value=max_idx, value=end_idx, step=1, key=edit_end_idx_key)
                    if st.button("Use End Date", key="btn_edit_end_idx"):
                        idx_match = df[df['time'].str[:10] == str(end_date)].index
                        if not idx_match.empty:
                            st.session_state[edit_end_idx_key] = idx_match[-1]
                        st.rerun()
